...
证据：C；本页以固定代码提交 `d99697a4194a5b2c27a6deace8a45c86e90e96fa` 为依据。阅读范围为 CLI 注册、模块声明、worker 材料、初始化、计划与产物、Windows 进程创建，以及私有快照、目录固定、读取预算和 Web 展示的相关符号；未变化部分复用前轮核对，哈希及边界见[版本证据](../evidence/versions.md)。源码可证实不等于本任务已运行生产测试，也不描述微信自身的内部架构。
