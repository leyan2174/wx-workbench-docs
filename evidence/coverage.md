...
当前源码基线为 `d99697a4194a5b2c27a6deace8a45c86e90e96fa`，本次增量仅核对单标签查询目标非空边界；私有快照、目录固定、读取预算、文章解析及入口展示复用此前基线证据。T只指文档自有合成检查；历史SILK封包检查不代表当前导出器执行规范化，维护方生产测试结果另记于[版本证据](versions.md)，不是本任务运行结果。
