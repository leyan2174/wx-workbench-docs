...
固定实现允许显式离线SNS/联系人数据库和缓存目录，但固定账号、配置、实际来源及其祖先目录。已有 `-wal`、`-shm`、`-journal` 文件保持Pin；原本缺失的侧车在复核时出现，也按来源变化拒绝。此机制不执行checkpoint，不保证所有实时WAL组合都可离线读取，不证明离线来源属于当前微信账号。

缓存候选按实际六字节头分类，每个V2候选都需受保护AES材料并对同一固定文件验证；失败不降级为跳过警告或明文回退。plain、XOR、V1与视频不因Store有AES就额外要求AES样本。`image_material_status`分别为无缓存的 `not_requested`、没有V2样本的 `unverified`、V2局部样本通过的 `verified_local_samples`；后者不代表帖子关联或整个账号完整。

无缓存分支不读取图片材料，也不创建空恢复对象，但仍在最终发布前复核SNS/联系人来源及Runtime/配置；有缓存时额外复核样本与材料revision。fresh在目录rename前检查，update逐文件persist前检查。后续失败保留此前已提交文件，不提供整目录事务或跨对象CAS。路径hash只是来源标签，不是账号归属证明；本地缓存与启发式关联均不能冒充完整远端历史。
