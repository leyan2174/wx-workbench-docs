...
项目单标签查询（如 `tag-members`）的目标在 `trim` 后必须非空；业务层和 daemon 在读取标签源前拒绝空白目标，MCP 在路由预检时拒绝。此校验不删除源数据中的空名标签，也不禁止联系人列表的空筛选；精确匹配优先、唯一模糊匹配及歧义报错的规则不变。依据固定版本的 `src/business/contacts.rs`、`src/daemon/query/mcp_contacts.rs` 和 `src/mcp/protocol.rs`，这是项目请求边界，不是微信标签存储格式约束。
